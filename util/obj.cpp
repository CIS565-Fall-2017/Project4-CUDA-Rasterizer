    ...
    for (int k = 0; k < (int) faces.size(); k++) {
        ...
        if (isConvex(faces[k]) == true) {
            ...
            for (int i = 2; i < (int) face.size(); i++) {
                ...
                BufPosvec.push_back(p0[1]);
                BufPosvec.push_back(p0[2]); //BufPosvec.push_back(1.0f);
                BufPosvec.push_back(p1[0]);
                BufPosvec.push_back(p1[1]);
                BufPosvec.push_back(p1[2]); //BufPosvec.push_back(1.0f);
                ...
                if (genNormals == false) {
                    ...
                    BufNorvec.push_back(normals[facenormal[0]][1]);
                    BufNorvec.push_back(normals[facenormal[0]][2]); //BufNorvec.push_back(0.0f);
                    BufNorvec.push_back(normals[facenormal[i - 1]][0]);
                    BufNorvec.push_back(normals[facenormal[i - 1]][1]);
                    BufNorvec.push_back(normals[facenormal[i - 1]][2]); //BufNorvec.push_back(0.0f);
                    BufNorvec.push_back(normals[facenormal[i]][0]);
                    BufNorvec.push_back(normals[facenormal[i]][1]);
                    BufNorvec.push_back(normals[facenormal[i]][2]); //BufNorvec.push_back(0.0f);
                } else {
...
void obj::addFace(vector<int> face) {
    ...
    float facexmax = points[face[0]][0];
    float faceymax = points[face[0]][1];
    float facezmax = points[face[0]][2];
    ...
    for (int i = 0; i < (int) face.size(); i++) {
        ...
        if (points[face[i]][0] < facexmin) {
            facexmin = points[face[i]][0];
        }
        if (points[face[i]][1] > faceymax) {
            faceymax = points[face[i]][1];
        }
        if (points[face[i]][1] < faceymin) {
            faceymin = points[face[i]][1];
        }
        if (points[face[i]][2] > facezmax) {
            facezmax = points[face[i]][2];
        }
        if (points[face[i]][2] < facezmin) {
            facezmin = points[face[i]][2];
        }
    }
...
void obj::recenter() {
    ...
    xmax = points[0][0] - center[0];
    xmin = points[0][0] - center[0];
    ymax = points[0][1] - center[1];
    ymin = points[0][1] - center[1];
    ...
    for (int i = 0; i < (int) faceboxes.size(); i++) {
        ...
        for (int j = 0; j < (int) face.size(); j++) {
            ...
            if (points[face[j]][0] < facexmin) {
                facexmin = points[face[j]][0];
            }
            if (points[face[j]][1] > faceymax) {
                faceymax = points[face[j]][1];
            }
            if (points[face[j]][1] < faceymin) {
                faceymin = points[face[j]][1];
            }
            if (points[face[j]][2] > facezmax) {
                facezmax = points[face[j]][2];
            }
            if (points[face[j]][2] < facezmin) {
                facezmin = points[face[j]][2];
            }
        }
        faceboxes[i][0]  =  facexmin;
        faceboxes[i][1]  =  faceymin;
        faceboxes[i][2]  =  facezmax;
        faceboxes[i][3]  = 1.0f;
        faceboxes[i][4]  =  facexmin;
        faceboxes[i][5]  =  faceymax;
        faceboxes[i][6]  =  facezmax;
        faceboxes[i][7]  = 1.0f;
        faceboxes[i][8]  =  facexmax;
        faceboxes[i][9]  =  faceymax;
        faceboxes[i][10] =  facezmax;
        faceboxes[i][11] = 1.0f;
        faceboxes[i][12] =  facexmax;
        faceboxes[i][13] =  faceymin;
        faceboxes[i][14] =  facezmax;
        faceboxes[i][15] = 1.0f;
        faceboxes[i][16] =  facexmin;
        faceboxes[i][17] =  faceymin;
        faceboxes[i][18] =  facezmin;
        faceboxes[i][19] = 1.0f;
        faceboxes[i][20] =  facexmin;
        faceboxes[i][21] =  faceymax;
        faceboxes[i][22] =  facezmin;
        faceboxes[i][23] = 1.0f;
        faceboxes[i][24] =  facexmax;
        faceboxes[i][25] =  faceymax;
        faceboxes[i][26] =  facezmin;
        faceboxes[i][27] = 1.0f;
        faceboxes[i][28] =  facexmax;
        faceboxes[i][29] =  faceymin;
        faceboxes[i][30] =  facezmin;
        faceboxes[i][31] = 1.0f;
    }
}
...
int obj::getBufNorsize() {
    return nbosize;
}

int obj::getBufIdxsize() {
    return ibosize;
}
